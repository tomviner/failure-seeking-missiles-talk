...
def search(bs, vs):
    ...
    try:
        return find(st.text(max_size=1), cond, settings=Settings(max_examples=100099))
    except (NoSuchExample, Unsatisfiable):
        return


# bs = [bs for bs in itertools.product(*3*((False, True),))]
bs = [list(map(str.isupper, tr)) for tr in 'Abc, aBc, ABc, abC, AbC, aBC, ABC'.split(', ')]
...
# print(find(st.text(max_size=3), cast_blows_up, settings=Settings(max_examples=1000909)))
